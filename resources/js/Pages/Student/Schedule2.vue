    ...
    <student-layout>
        <div class="card mb-3" id="schedule-navigator">
            <div class="card-body d-flex justify-content-center">
                <section>
                    <button class="navigator" @click="previousWeek"><i class="fa-solid fa-arrow-left" title="<"></i></button>
                    <span class="date mx-4">{{
                            this.$moment(dates[0]).format('LL')
                        }} - {{ this.$moment(dates[dates.length - 1]).format('LL') }}</span>
                    <button class="navigator" @click="nextWeek"><i class="fa-solid fa-arrow-right" title=">"></i></button>
                </section>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        nextWeek() {
            ...
            this.$inertia.get(this.route('student.schedule'), { 'week': week }, { replace: true, preserveState: true, preserveScroll: true })
        },
        previousWeek() {
            let weekNumber = this.week.split('-')[1];
            weekNumber--;
