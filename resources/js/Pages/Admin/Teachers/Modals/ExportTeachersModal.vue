<template>
    <ShowModal :open="openModal" header="Exporteer docenten" @close="close">
        <p class="text-danger">Let op! Dit kan enige tijd duren.</p>

        <button class="btn btn-success" @click="exportTeachers" :disabled="loading">
            Exporteer
...
export default {
    ...
    methods: {
        ...
        exportTeachers() {
            this.loading = true;
            axios.get(route('admin.teachers.export'), { responseType: 'blob' })
                .then(response => {
                    const blob = new Blob([response.data], { type: 'application/xlsx' });
                    const link = document.createElement('a');
                    link.href = URL.createObjectURL(blob);
                    link.download = `Teachers_${this.$dayjs().format('DD-MM-YYYY')}.xlsx`;
                    link.click();
